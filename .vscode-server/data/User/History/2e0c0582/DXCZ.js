...
//---------------------------------------------------------------------------POST REQUESTS:---------------------------------------------------------------------------
app.post('/newstudent/:name/:title/:class/:section/:rollid', (req, res) => {
	pool.getConnection()
		.then(conn => {
		
			conn.query("INSERT INTO student (`NAME`, `TITLE`, `CLASS`, `SECTION`, `ROLLID`) VALUES ([req.params.name], [req.params.title], [req.params.class], [req.params.section], [req.params.rollid])", [req.params.agentCode])
			.then((rows) => {
			  console.log(rows);
			  res.setHeader('Content-Type', 'application/json');
			  conn.end();
