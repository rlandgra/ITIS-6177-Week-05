...
//---------------------------------------------------------------------------PUT REQUESTS:---------------------------------------------------------------------------
app.put('/agent/:agent_code', (req, res) => {
	pool.getConnection()
		.then(conn => {
			conn.query("UPDATE agents SET `AGENT_NAME`=?, `WORKING_AREA`=?, `COMMISSION`=?, `PHONE_NO`=?, `COUNTRY`=? WHERE `AGENT_CODE` =?", [req.query.agent_name, req.query.working_area, req.query.commission, req.query.phonenumber, req.query.country, req.params.agent_code])
			.then((rows) => {
				if (rows && rows.affectedRows > 0){
				res.send('Agent has been updated');
			} 
